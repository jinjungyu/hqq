...
lib.define("vecquant3matmul_faster_old_256(Tensor vec, Tensor mat, Tensor scales, Tensor zeros, int groupsize, int vec_height) -> Tensor")

def vecquant3matmul_faster_old_256(vec: Tensor, mat: Tensor, scales: Tensor, zeros: Tensor, groupsize: int, vec_height: int) -> Tensor:
    # matvec
    ...
    input_args = [vec, mat, scales, zeros, groupsize, vec_height]
    import code; code.interact('vecquant3matmul_faster_old_256', local=dict(globals(), **locals()))
    # import autogptq_cuda_256
    # autogptq_cuda_256.vecquant3matmul_faster_old_256(*input_args)
    opcheck(torch.ops.autogptq.vecquant3matmul_faster_old_256.default, input_args)
    return torch.ops.autogptq.vecquant3matmul_faster_old_256.default(
        vec=vec,
        mat=mat,
        scales=scales,
        zeros=zeros,
...
class AutoGPTQLinear(nn.Module):
    QUANT_TYPE = "cuda-old"
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if x.shape[0] < self.kernel_switch_threshold:
            half_dim = self.infeatures // 2
            out = self.matvec(x, self.qweight, self.scales, self.qzeros, self.group_size, half_dim)
        else:
...
def patch_hqq_to_gptq(layer, patch_params):
    ...
    bias = hqq_layer.bias
    gptq_layer = AutoGPTQLinear(nbits, 
                                group_size, 
                                infeatures, 
                                outfeatures, 
                                bias).to(device)
